...
- **Core Loop**: `main.py` receives proxied requests. An LLM (Groq) acts as a router, classifying intent into Quant, News, Stock Screening, or Comparison.
- **Data Fetchers**: `fetcher.py`, `nse_client.py`, and provider adapters pull live quotes or local Supabase data.
- **Synthesis**: The LLM synthesizes a markdown answer, while structured `quant_data` from `/api/quant` conventions is returned alongside it for precise frontend rendering.

## Data Flow
1. User types in chat -> `frontend/app/api/chat` -> `backend/api/chat`.
2. Backend LLM routes intent -> fetches from Supabase / NSE / fallback YFinance / News RSS.
...
- **YFinance**: Fallback stock market data (often rate-limited on Render).
